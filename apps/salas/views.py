...
def editar_sala(request, id_sala):
    ...
    if True:
        if request.method == 'POST':
            ...
            return render(request, 'salas/editar_sala.html', {'form': form,'itemlist': list(range(0,26)),'lista_sillas': lista_sillas})
    # En caso de que el usuario no sea gerente se redirije al home y se muestra mensaje de error
    else:
        messages.error(request, 'No estas autorizado para realizar esta acción')
        return redirect('accounts:home')
...
def ver_sala(request,id_sala):
    ...
    return render(request, 'salas/consultar_sala.html', {'sala': listar_sala(id_sala),'itemlist': list(range(0,26)),'lista_sillas': lista_sillas})
